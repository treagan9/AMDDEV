...
var whyDefaults = {
  ...
  body: 'Once you experience healthcare without the waiting, the rushing and the runaround, it\'s hard to go back.',
  items: [
...
function WhyUs() {
  ...
  var c = usePageContent('home', 'whyUs', whyDefaults);
  var items = c.items || whyDefaults.items;
    ...
    <Box py={{ base: 'sectionMobile', md: 'section' }} ref={ref} sx={{ background: 'linear-gradient(to bottom, #FFFFFF 0%, #FAF8F2 38%, #F7F3EB 70%, #F5F2EB 100%)' }}>
      <Box maxW="98%" mx="auto" px={{ base: 6, md: 4 }}>
        ...
        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={5} maxW={{ base: '100%', lg: '60%' }} mx="auto">
          {items.map(function (reason, i) {
            return (
              <MotionBox key={reason.title} initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: 0.2 + i * 0.1 }} bg="white" borderRadius="card" borderWidth="1px" borderColor="rgba(60,50,40,0.07)" boxShadow="0 8px 32px rgba(60,50,40,0.05)" py={{ base: 8, md: 10 }} px={{ base: 7, md: 10 }} sx={{ transition: 'transform 0.3s ease, box-shadow 0.3s ease' }} _hover={{ transform: 'translateY(-4px)', boxShadow: '0 16px 44px rgba(60,50,40,0.09)' }}>
