...
var testDefaults = {
  ...
  body: 'Real stories from members who left the waiting rooms and the runaround behind.',
  reviews: [
...
function Testimonials() {
  ...
  var c = usePageContent('home', 'testimonials', testDefaults);
  var reviews = c.reviews || testDefaults.reviews;
      ...
      <Box maxW={{ base: '100%', lg: '70%' }} mx="auto" px={{ base: 6, md: 4 }}>
        <Flex direction={{ base: 'column', lg: 'row' }} align={{ base: 'flex-start', lg: 'center' }} gap={{ base: 10, lg: 16 }}>
          <MotionBox initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} w={{ base: '100%', lg: '42%' }} flexShrink={0}>
            ...
            <Text as="h2" fontFamily="heading" fontSize={{ base: '3xl', md: '4xl', lg: '5xl' }} fontWeight={700} color="brand.slate" lineHeight={1.1} mb={5}>{c.heading}</Text>
            <Text fontSize={{ base: 'md', md: 'lg' }} color="brand.body" lineHeight={1.8} maxW="420px">{c.body}</Text>
          </MotionBox>
