  ...
  return (
    <Box>
      <Flex align="center" gap={4} mb={8}>
        ...
        <Box flex={1}>
          <Text fontFamily="heading" fontSize="3xl" fontWeight={700} color="#2D2D2D">{lead.first_name} {lead.last_name}</Text>
          <Text fontSize="md" color="#6B6560">Lead from {lead.source} form &middot; {new Date(lead.created_at).toLocaleDateString()}</Text>
        </Box>
        {lead.status !== 'enrolled' && (
          <Button onClick={convertToPatient} bg="#1B3A34" color="white" borderRadius="8px" size="md" leftIcon={<HiOutlineUserAdd size={18} />} _hover={{ bg: '#234840' }} isLoading={converting} loadingText="Converting...">Convert to patient</Button>
        )}
      </Flex>
      ...
      <Flex direction={{ base: 'column', lg: 'row' }} gap={8}>
        <Box flex={1}>
          <Box bg="white" borderRadius="18px" border="1px solid" borderColor="#E8E2D8" p={8} mb={6}>
            ...
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={6}>
              <Box>
                <Text fontSize="sm" fontWeight={600} color="#6B6560" mb={1}>Email</Text>
                <Flex align="center" gap={2}>
                  <HiOutlineMail size={16} color="#9A9590" />
                  <Text fontSize="md" color="#2D2D2D">{lead.email || '—'}</Text>
                </Flex>
              </Box>
              <Box>
                <Text fontSize="sm" fontWeight={600} color="#6B6560" mb={1}>Phone</Text>
                <Flex align="center" gap={2}>
                  <HiOutlinePhone size={16} color="#9A9590" />
                  <Text fontSize="md" color="#2D2D2D">{lead.phone || '—'}</Text>
                </Flex>
              </Box>
              <Box>
                <Text fontSize="sm" fontWeight={600} color="#6B6560" mb={1}>Location</Text>
                <Text fontSize="md" color="#2D2D2D">{lead.location || '—'}</Text>
              </Box>
              <Box>
                <Text fontSize="sm" fontWeight={600} color="#6B6560" mb={1}>Interest</Text>
                <Text fontSize="md" color="#2D2D2D">{lead.interest || '—'}</Text>
              </Box>
            </SimpleGrid>
            {lead.message && (
              <Box mt={6}>
                <Text fontSize="sm" fontWeight={600} color="#6B6560" mb={2}>Message</Text>
                <Box bg="#FAFAF7" borderRadius="8px" p={4} border="1px solid" borderColor="#E8E2D8">
                  <Text fontSize="md" color="#2D2D2D" lineHeight={1.7}>{lead.message}</Text>
                </Box>
              </Box>
            )}
          </Box>
          ...
          {submission && (
            <Box bg="white" borderRadius="18px" border="1px solid" borderColor="#E8E2D8" p={8} mb={6}>
              <Flex justify="space-between" align="center" mb={6}>
                <Text fontFamily="heading" fontSize="lg" fontWeight={700} color="#2D2D2D">Form submission snapshot</Text>
        ...
        <Box w={{ base: '100%', lg: '320px' }} flexShrink={0}>
          <Box bg="white" borderRadius="18px" border="1px solid" borderColor="#E8E2D8" p={6} mb={6}>
            ...
            <Select value={lead.status} onChange={function (e) { updateField('status', e.target.value); }} bg="#FAFAF7" border="1px solid" borderColor="#D5D0C8" borderRadius="8px" h="48px" fontSize="md" _focus={{ borderColor: '#C4A265' }}>
              {statusOptions.map(function (s) {
                return <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>;
              })}
            </Select>
            <Box mt={4}>
              <Badge colorScheme={statusColors[lead.status] || 'gray'} borderRadius="full" px={3} py={1} fontSize="sm" textTransform="capitalize">{lead.status.replace(/_/g, ' ')}</Badge>
            </Box>
          </Box>

          <Box bg="white" borderRadius="18px" border="1px solid" borderColor="#E8E2D8" p={6} mb={6}>
            <Text fontFamily="heading" fontSize="lg" fontWeight={700} color="#2D2D2D" mb={4}>Assign physician</Text>
            <Select value={lead.assigned_to || ''} onChange={function (e) { updateField('assigned_to', e.target.value || null); }} bg="#FAFAF7" border="1px solid" borderColor="#D5D0C8" borderRadius="8px" h="48px" fontSize="md" _focus={{ borderColor: '#C4A265' }}>
